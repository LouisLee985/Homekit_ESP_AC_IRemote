...
// coolix Fan: 7 (Fixed),Fan: 4 (Min),Fan: 2 (Med),Fan: 1 (Max),Fan: 5 (Auto)	
	switch(newsp){
		case 5:	ac.setFan(ac.convertFan(stdAc::fanspeed_t::kAuto));
		break;
		case 4:	ac.setFan(ac.convertFan(stdAc::fanspeed_t::kMax));
		break;
		case 3:	ac.setFan(ac.convertFan(stdAc::fanspeed_t::kMedium));
		break;
		case 2:	ac.setFan(4);
		break;
		case 1:	ac.setFan(7);
...
void update_status(){
	...
	if (ac_active.value.bool_value==false){	new_current_state = HOMEKIT_UINT8(0); }
	else{	
